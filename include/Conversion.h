...
namespace skylens {
  ...
  class Conversion {
  public:
    /// Compute flux \f$f\ [\mathrm{ergs/(s~cm^2~Hz)}]\f$
    ...
    /// when integrated for \p time \f$t\ [\mathrm{s}]\f$ with a telescope \p tel.
    /// \f[n=\frac{\pi D^2 t f}{4h}\int{\frac{T(\lambda)}{\lambda}d\lambda},\f]
    /// where \f$T(\lambda)\f$ and \f$D\ [\mathrm{cm}]\f$ describes the total 
    /// transmission and diameter of the \p tel 
    /// and \f$h = \mathrm{6.6260\ erg~s}\f$ is Planck's constant.
    ...
    /// Compute flux \f$f\ [\mathrm{ergs/(s~cm^2~Hz)}]\f$ from \p ADU and
